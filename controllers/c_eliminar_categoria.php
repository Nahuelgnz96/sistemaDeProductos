...
if (isset($_POST['categoriasAEliminar'])) {
    ...
    if (!empty($categoriasAEliminarStr)) {
        $sqlEliminar = "DELETE FROM categorias WHERE ID_Categoria IN ($categoriasAEliminarStr)";
        ...
        if ($conexion->query($sqlEliminar) === TRUE) {
            ...
            echo "Error al eliminar Categorias: " . $conexion->error;
        }
    } else {
        // La cadena de categorías a eliminar está vacía
        echo "Advertencia: No se proporcionaron Categorias válidas para eliminar.";
    }
} else {
    // No se recibió la variable 'categoriasAEliminar' en la solicitud POST
    echo "Advertencia: No se proporcionaron Marcas para eliminar4.";
}
