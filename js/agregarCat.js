...
document.getElementsByName('btnagregar')[0].addEventListener('click', function () {
    ...
    if (categorias.length > 0) {
        ...
        fetch('../controllers/c_agregar_categoria.php', {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                Swal.fire({
                    title: 'Categorias agregadas: ' + data.cantidad_categorias,
                    icon: 'success',
...
function obtenerDatosFilasCompletas() {
    ...
    for (var i = 1; i <= 5; i++) {
        var categoria = {
            Categoria: document.getElementsByName('Categoria' + i)[0].value.trim()
        };
